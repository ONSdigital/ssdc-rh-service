...
class UacUpdateReceiverIT {
    ...
    public void newCopyOfActiveUAacDoesNotOverwriteLaunchData() throws UacNotFoundException {
        ...
        // Now fire in a case update with a new Barcode, then a 'double delivered UAC' - technically possible, very unlikely
        ...
        // without this method of changing and checking the QUD we'll often get the old already correct UAC Udpdate
        uacOpt = fireStorePoller.getUACByHashAndQID(uacUpdateDTO.getUacHash(), "SomethingToUpdateToCheckItsUpdated");
        Assertions.assertTrue(uacOpt.isPresent());
