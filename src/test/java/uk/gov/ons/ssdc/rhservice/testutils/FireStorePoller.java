...
public class FireStorePoller {
    ...
    @Retryable(
            ...
            maxAttempts = 5,
            backoff = @Backoff(delay = 1000))
    public Optional<CollectionExerciseUpdateDTO> getCollectionExerciseById(String collectionExcerciseId) throws CollectionExerciseNotFoundException {

        Optional<CollectionExerciseUpdateDTO> collexOpt = collectionExerciseRepository.readCollectionExerciseUpdate(collectionExcerciseId);

        if (collexOpt.isPresent()) {
            return collexOpt;
        } else {
            throw new CollectionExerciseNotFoundException("Collection Exercise Not found: " + collectionExcerciseId);
        }
    }
