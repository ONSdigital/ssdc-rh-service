...
public class FireStorePoller {
    ...
    @Retryable(
            ...
            maxAttempts = 5,
            backoff = @Backoff(delay = 1000))
    public Optional<UacUpdateDTO> getUacByHash(String hash, boolean expectedActive) throws UacNotFoundException {
        ...
        if (uacUpdateOpt.isPresent()) {
            if (uacUpdateOpt.get().isActive() != expectedActive) {
                throw new UacNotFoundException("UAC found but not in right active state, expected: " + expectedActive);
            }
