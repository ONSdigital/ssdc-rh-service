...
public class UacService {
    ...
    public UacOr4xxResponseEntity getUac(String uacHash) throws RuntimeException {
        ...
        uacOr4xxResponseEntity.setCollectionExerciseUpdateDTO(getCollectionExerciseFromCase(uacOr4xxResponseEntity.getCaseUpdateDTO()));
    ...
    private CollectionExerciseUpdateDTO getCollectionExerciseFromCase(CaseUpdateDTO caseUpdateDTO) {
        ...
        if (StringUtils.isEmpty(collectionExerciseId)) {
            throw new RuntimeException("Case has no collection exercise id");
        }

        return collectionExerciseRepository.readCollectionExerciseUpdate(collectionExerciseId).orElseThrow(() -> new RuntimeException("Colection Exercise Not Found for UAC"));
    }
}
