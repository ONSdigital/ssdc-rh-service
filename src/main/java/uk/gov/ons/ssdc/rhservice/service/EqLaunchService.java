...
public class EqLaunchService {
  ...
  public EqLaunchService(
      KeyStore keyStore, UacRepository uacRepository, CaseRepository caseRepository) {
  ...
  Map<String, Object> createPayloadMap(
    ...
    //TODO: what is all this stuff about! ROLE_FLUSHER ??
    // Maybe todo with Wanting to 'flush' data out where they've launched or for some other reason
    // At min not appropriate for this ticket, remove all concept of Roles from code
    if (role == null || !role.equals(ROLE_FLUSHER)) {
      ...
      payload.computeIfAbsent("language_code", (k) -> eqLaunchCoreData.getLanguage().getIsoLikeCode());
